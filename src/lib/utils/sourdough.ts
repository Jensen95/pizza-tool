...
import type { PlannedIngredient } from './dough-planner';
...
export type SourdoughPlanWarning = 'starter-exceeds-water';
...
export function planSourdough(input: SourdoughPlanInput): SourdoughPlan | null {
	...
	const addedWater = totalWater - waterInStarter;

	const warnings: SourdoughPlanWarning[] = [];
	if (addedWater < 0) warnings.push('starter-exceeds-water');
